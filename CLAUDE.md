...
**Env vars** (`.env.local`): `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, `TICKETMASTER_API_KEY` (optional — see Live dates).
...
> **Never derive an aggregate from a `.limit()`ed list.** `counts()` in `src/app/app/stats/page.tsx` caps `get_play_counts` at 50 for the top-tracks table. `totalPlays` used to be `all.reduce(...)` over that list, so "plays" silently counted only the user's 50 most-played tracks and drifted further from the truth the more they listened; `StatsView`'s "Unique songs" card read `all.length` and so pinned at 50 forever. Totals now come from `totalPlayCount()`, a `count: "exact", head: true` query over `play_history` (one row per play): exact, uncapped, and it transfers no rows. The Unique songs card was dropped rather than fixed. The one surviving use of the capped length is the Explorer badge, which only asks `>= 50` — a saturating counter answers that correctly, and it is commented as such. The identical bug existed in the Expo app and is fixed there too.

**Timezone caveat.** `get_play_days` / `get_play_hours` are still called here with a hardcoded `p_tz: "UTC"` (`src/app/app/stats/page.tsx`, `src/app/api/wrapped/route.tsx`), so peak hours and streak day-boundaries are UTC for everyone — a US evening session lands on the next day. The Expo app now passes the **device's** zone, so the two platforms disagree for anyone outside UTC. Fixing it here needs the user's timezone from somewhere a Server Component can read: a `timezone` column on `profiles`, populated the way `country` already is.
