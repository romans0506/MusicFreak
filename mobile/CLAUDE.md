...
The one section that isn't period-scoped server-side is the hour histogram: `get_play_hours` takes only a timezone, so Today/Week/Month are bucketed **on the device** from raw `played_at` timestamps (one 30-day fetch covers all three), while All Time keeps the RPC. Bucketing is in **UTC**, matching the RPC's `p_tz: "UTC"`, so the peak hour can't jump between pages — both are wrong for anyone off UTC, which is a single fixable bug rather than a per-page disagreement. Adding `p_since` to `get_play_hours` would let all four come off the server; the device-side fetch is capped at 5000 rows, so no headline number is ever derived from it.
