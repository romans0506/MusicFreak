// Hard music trivia about the biggest artists in the world — facts about the
// artists themselves, their songs, albums and personal lives. Replaces the old
// "guess your own top track" quiz, which was trivially easy.
//
// AUTHORING RULE: options[0] is ALWAYS the correct answer. The quiz route
// (src/app/api/quiz/generate) shuffles the four options before sending, so
// order here doesn't leak the answer. Keep the three distractors plausible but
...
// Maps each question's id prefix to the artist whose photo to show alongside it.
// The subject is named in (almost) every question, so showing their photo never
// gives away the answer. The few "identify this band" questions — where a photo
// WOULD spoil it (radio/genesis/police) — are intentionally omitted, so the UI
// shows a question-mark placeholder instead.
const PHOTO_ARTIST: Record<string, string> = {
  beatles: "The Beatles", queen: "Queen", mj: "Michael Jackson", elvis: "Elvis Presley",
  madonna: "Madonna", bey: "Beyoncé", jayz: "Jay-Z", rih: "Rihanna", ts: "Taylor Swift",
  drake: "Drake", em: "Eminem", ye: "Kanye West", gaga: "Lady Gaga", adele: "Adele",
  ed: "Ed Sheeran", cold: "Coldplay", u2: "U2", nirv: "Nirvana", pf: "Pink Floyd",
  lz: "Led Zeppelin", dylan: "Bob Dylan", bowie: "David Bowie", prince: "Prince",
  wh: "Whitney Houston", mc: "Mariah Carey", abba: "ABBA", wknd: "The Weeknd",
  bruno: "Bruno Mars", jb: "Justin Bieber", jt: "Justin Timberlake", brit: "Britney Spears",
  spice: "Spice Girls", bts: "BTS", marley: "Bob Marley", shak: "Shakira",
  billie: "Billie Eilish", ari: "Ariana Grande", hs: "Harry Styles", amy: "Amy Winehouse",
  oasis: "Oasis", met: "Metallica", acdc: "AC/DC", gnr: "Guns N' Roses",
  beach: "The Beach Boys", marvin: "Marvin Gaye", snoop: "Snoop Dogg", dre: "Dr. Dre",
  pac: "2Pac", big: "The Notorious B.I.G.",
}

/** Artist name to show a photo for on this question, or null (→ placeholder). */
export function photoArtistFor(id: string): string | null {
  return PHOTO_ARTIST[id.split("-")[0]] ?? null
}
